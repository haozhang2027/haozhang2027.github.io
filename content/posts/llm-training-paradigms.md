...
### 核心原理
**Next-Token Prediction (NTP)**：给定上文 $x_{\lt t}$，最大化下一个 token $x_t$ 的概率。

$$\mathcal{L}_{\text{LM}} = -\frac{1}{T}\sum_{t=1}^{T} \log P_\theta(x_t \mid x_{\lt t})$$
...
$$r_{\text{seq}} = \frac{\pi_\theta(y|x)}{\pi_{\text{old}}(y|x)} = \prod_t \frac{\pi_\theta(y_t|x, y_{\lt t})}{\pi_{\text{old}}(y_t|x, y_{\lt t})}$$
